class CreateReviewUseCase {
  ...
  async execute({ userId, petId, rating, comment, recommend }) {
    ...
    if (Number.isNaN(rNum) || rNum < 1 || rNum > 5) throw new Error("rating deve ser entre 1 e 5");
    ...
    this.notificationGateway.emit("review:created", {
      reviewId: saved.id, petId: saved.petId,
      userId: saved.userId, rating: saved.rating,
    });
