...
describe("ListAdminsUseCase", () => {
  ...
  beforeEach(() => {
    adminRepository = { findAll: jest.fn().mockResolvedValue([{ id: "a1", name: "Admin 1" }]) };
    cacheGateway = { get: jest.fn().mockResolvedValue(null), set: jest.fn().mockResolvedValue(undefined) };
  ...
  it("busca no banco e armazena no cache quando MISS", async () => {
    ...
    expect(cacheGateway.set).toHaveBeenCalledWith(expect.stringContaining("admin:list"), expect.any(String), 60);
  ...
  it("lanca erro sem adminRepository", () => { expect(() => new ListAdminsUseCase({ cacheGateway })).toThrow("adminRepository e obrigatorio"); });
  it("lanca erro sem cacheGateway", () => { expect(() => new ListAdminsUseCase({ adminRepository })).toThrow("cacheGateway e obrigatorio"); });
});
