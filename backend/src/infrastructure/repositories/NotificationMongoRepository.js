...
class NotificationMongoRepository {
  ...
  async findByUserId({ userId, onlyUnread = false, page = 1, limit = 20 }) {
    ...
    const docs = await NotificationModel.find(query).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit);
  ...
  _toPlain(doc) {
    return { id: doc._id.toString(), userId: doc.userId.toString(), type: doc.type, title: doc.title, message: doc.message, isRead: doc.isRead, metadata: doc.metadata, createdAt: doc.createdAt };
  }
}
