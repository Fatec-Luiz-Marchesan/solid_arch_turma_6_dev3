class Location {
  constructor({ id, name, latitude, longitude, type, referenceId, createdAt }) {
    ...
    if (!['pet', 'event', 'user'].includes(type)) {
      throw new Error('Location.type deve ser pet, event ou user');
    }
    this.id = id;
    this.name = name.trim();
  ...
  distanceTo(otherLat, otherLng) {
    const R = 6371;
    const dLat = (otherLat - this.latitude) * Math.PI / 180;
    const dLng = (otherLng - this.longitude) * Math.PI / 180;
