...
describe("DeleteAdoptionUseCase", () => {
  ...
  beforeEach(() => {
    adoptionRepository = {
      findById: jest
        .fn()
        .mockResolvedValue({ id: "a1", adopterId: "u1", status: "active" }),
  ...
  it("lanca erro se requester nao e o adotante", async () => {
    await expect(
      usecase.execute({ id: "a1", requesterId: "outro-user" }),
  ...
  it("lanca erro ao tentar cancelar adocao ja concluida", async () => {
    ...
    await expect(
      usecase.execute({ id: "a1", requesterId: "u1" }),
    ).rejects.toThrow("concluida");
  });
  it("rejeita sem id", async () => {
    await expect(usecase.execute({ requesterId: "u1" })).rejects.toThrow(
      "id e obrigatorio",
    );
  });
  it("lanca erro sem adoptionRepository", () => {
    expect(() => new DeleteAdoptionUseCase({})).toThrow(
