...
class CreateAdoptionUseCase {
  constructor({ adoptionRepository, logger }) {
    if (!adoptionRepository)
      throw new Error("adoptionRepository e obrigatorio");
  ...
  async execute({ petId, adopterId, notes }) {
    ...
    try {
      ...
      if (existing) {
        this.logger.warn("Tentativa de adotar pet ja adotado", {
          petId,
          adopterId,
        });
      ...
      const saved = await this.adoptionRepository.create(adoption);
      this.logger.info("Adocao criada com sucesso", {
        adoptionId: saved.id,
        petId: saved.petId,
        adopterId: saved.adopterId,
      });
      return saved;
    } catch (err) {
      if (err.message !== "Este pet ja foi adotado") {
        this.logger.error("Erro ao criar adocao", err);
      }
      throw err;
    }
  }
