class UploadFileUseCase {
  constructor({ storageGateway, uploadRepository }) {
    if (!storageGateway) throw new Error("storageGateway e obrigatorio");
    if (!uploadRepository) throw new Error("uploadRepository e obrigatorio");
    this.storageGateway = storageGateway;
    this.uploadRepository = uploadRepository;
  }
  async execute({ file, ownerId, purpose = "general" }) {
    if (!file) throw new Error("Arquivo e obrigatorio");
