...
describe('ListReviewsUseCase', () => {
  ...
  beforeEach(() => {
    reviewRepository = {
      findAll: jest.fn().mockResolvedValue({
        reviews: [{ id: 'r1', rating: 4 }, { id: 'r2', rating: 5 }],
        total: 2,
      }),
    };
  ...
  it('rejeita limit maior que 100', async () => {
    await expect(usecase.execute({ limit: 200 })).rejects.toThrow('entre 1 e 100');
  });

  it('lanca erro sem reviewRepository', () => {
    expect(() => new ListReviewsUseCase({})).toThrow('reviewRepository e obrigatorio');
  });
});
...
describe('GetReviewUseCase', () => {
  ...
  it('lista reviews de um pet', async () => {
    reviewRepository.findByPetId.mockResolvedValue([{ id: 'r1' }, { id: 'r2' }]);
